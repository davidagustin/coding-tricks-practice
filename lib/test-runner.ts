...
export async function runTests(
  ...
  const timeoutPromise = new Promise<TestRunnerResult>((resolve) => {
    setTimeout(() => {
      resolve({
        allPassed: false,
        results: [],
  ...
  // Create the actual test execution promise
  const testExecutionPromise = executeTests(userCode, testCases, solutionFunctionName);

  // Race between timeout and test execution
  return Promise.race([testExecutionPromise, timeoutPromise]);
}
