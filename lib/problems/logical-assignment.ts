  ...
  testCases: [
    {
      ...
      description: 'setDefaults fills null values but keeps existing ones',
    },
    {
      input: [{ timeout: 0, debug: false }],
      expectedOutput: { timeout: 0, retries: 3, debug: false, name: 'default' },
      description: 'setDefaults with ??= preserves falsy values like 0 and false',
    },
    {
      input: [{ name: 'john', email: 'JOHN@EXAMPLE.COM', age: 25 }],
      expectedOutput: { name: 'JOHN', email: 'john@example.com', age: 50 },
      description: 'transformIfPresent transforms all truthy values',
    },
    {
      input: [{ name: '', email: null, age: 0 }],
      expectedOutput: { name: '', email: null, age: 0 },
      description: 'transformIfPresent skips falsy values with &&=',
    },
    {
      input: [['hello', 'world', 'hello']],
      expectedOutput: { hello: 2, world: 1 },
      description: 'createWordCounter counts words correctly using ??=',
    },
  ],
  hints: [
