  ...
  it('should not have uncommented .then(console.log) calls in starter code', () => {
    problems.forEach((problem) => {
      ...
      if (hasUncommentedThen) {
        console.error(
          `Problem "${problem.title}" (${problem.id}) has uncommented .then(console.log) in starter code`
        );
      }
    ...
    problems.forEach((problem) => {
      ...
      lines.forEach((line, index) => {
        ...
        if (trimmed.match(/\.catch\s*\(\s*console\.(error|log|warn)\s*\)\s*;?\s*$/)) {
          console.error(
            `Problem "${problem.title}" (${problem.id}) line ${index + 1} has uncommented .catch(console.error): ${trimmed}`
          );
    ...
    problems.forEach((problem) => {
      ...
      lines.forEach((line, index) => {
        ...
        if (immediateExecutionPattern.test(trimmed)) {
          ...
          console.warn(
            `Problem "${problem.title}" (${problem.id}) line ${index + 1} may execute immediately: ${trimmed.substring(0, 50)}...`
          );
